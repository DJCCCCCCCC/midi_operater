...
// plus a chat-bar MIDI upload control.
//
...
return {
  apply(ctx) {
    ...
    ctx.effect(() => styles.insert(
      ...
      + '.dsh-midi-upload { display: inline-flex; align-items: center; gap: 6px; cursor: pointer; }'
      + '.dsh-midi-upload-btn { font-size: 12px; color: #4a9eff; cursor: pointer; }'
      + '.dsh-midi-upload-btn:hover { text-decoration: underline; }'
      + '.dsh-midi-upload-status { font-size: 11px; color: #9a9aa2; }'
      + '.dsh-midi-upload-done { font-size: 11px; color: #5abf7a; max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }'
      + '.dsh-midi-upload-err { font-size: 11px; color: #d05a5a; }',
    ))
    ...
    // (e.g. "Violin I" naming track followed by its flute track).
    ...
    function MidiUpload() {
      const [state, setState] = React.useState({ status: 'idle', path: null, error: null })
      ...
      function onFile(e) {
        ...
        reader.onload = function () {
          ...
          if (!base64) {
            setState({ status: 'error', path: null, error: '读取文件失败' })
            return
          }
          setState({ status: 'uploading', path: null, error: null })
          host.call('midi-upload', { name: file.name, base64: base64 }).then(function (res) {
            setState({ status: 'done', path: res.path, error: null })
          }).catch(function (err) {
            setState({ status: 'error', path: null, error: String((err && err.message) || err) })
          })
        }
      ...
      return React.createElement('label', { className: 'dsh-midi-upload', title: '上传 MIDI 文件到工作区' },
        React.createElement('span', { className: 'dsh-midi-upload-btn' }, '上传 MIDI'),
        React.createElement('input', { type: 'file', accept: '.mid,.midi', style: { display: 'none' }, onChange: onFile }),
        state.status === 'uploading' ? React.createElement('span', { className: 'dsh-midi-upload-status' }, '上传中…') : null,
        state.status === 'done' ? React.createElement('span', { className: 'dsh-midi-upload-done', title: state.path }, '✓ ' + state.path) : null,
        state.status === 'error' ? React.createElement('span', { className: 'dsh-midi-upload-err' }, state.error) : null,
      )
    }
    ...
    slots.inject('conversation.input.left', function () {
      return slots.register(
        { name: 'conversation.input.left', id: 'midi-upload', order: 0, label: '上传 MIDI' },
        function () {
          return React.createElement(MidiUpload)
        },
      )
    })
  },

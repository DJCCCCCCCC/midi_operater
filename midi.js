// Shared MIDI engine for dsh-midi-plugin. This is a plain Node ESM module that
// imports @tonejs/midi directly and performs one operation per `run(op)` call.
//
// It is imported two ways:
//   - index.js (the persistent host bundle) calls run() in-process.
//   - worker.mjs (the dynamic piano-roll companion) wraps it behind stdin/stdout,
//     because dynamic Cordis plugin code cannot `import` npm packages.
//
// ops:
//   { op: 'read', path, mode: 'summary' | 'full' }
//   { op: 'write', path, midi }
//   { op: 'transpose', path, semitones, trackIndex?, outputPath? }
//   { op: 'quantize', path, subdivisions?, outputPath? }
//   { op: 'tempo', path, bpm, outputPath? }
//   { op: 'chords', path, epsilon? }
//   { op: 'upload', path, base64 }
//   { op: 'download', path, trackIndex? }
import { readFile, writeFile, mkdir, readdir, stat, unlink } from 'node:fs/promises'
import { dirname, join, basename } from 'node:path'
import toneMidi from '@tonejs/midi'

// @tonejs/midi is CommonJS/UMD: its default export carries { Midi, Track, Header }.
const { Midi } = toneMidi
...
/** Perform one MIDI operation and return its JSON-serializable result. */
